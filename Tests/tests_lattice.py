...
from TBG import tbg, hex_lattice
...
class test_twist_constants(unittest.TestCase):
    ...
    def test_valid_parameters(self):
        ...
        expected_N_raw = constants.np.sqrt(5**2 + 3*1**2)  # sqrt(28)
    ...
    def test_a_divisible_by_3(self):
        ...
        self.assertAlmostEqual(alpha,  4 * constants.np.pi, places=10)
    ...
    def test_parameter_validation(self):
        ...
        with self.assertRaises(constants.physics_parameter_error):
            validate_ab(2, 0)  # b cannot be zero
        
        with self.assertRaises(constants.physics_parameter_error):
            validate_ab(4, 6)  # not coprime (gcd = 2)
        
        with self.assertRaises(constants.physics_parameter_error):
            validate_ab(3, 5)  # |b| > a

class test_tbg_construction(unittest.TestCase):
    """Test TBG system construction."""
    
    def setUp(self):
        """Create a small TBG system for testing."""
        self.tbg_system = tbg(maxsize_n=2, maxsize_m=2, a=5, b=1, 
                      interlayer_dist_threshold=2.0,
                       intralayer_dist_threshold=2.0,unit_cell_radius_factor=1.0)
    
    def test_tbg_creation(self):
        """Test that TBG system is created successfully."""
        self.assertEqual(self.tbg_system.a, 5)
        self.assertEqual(self.tbg_system.b, 1)
        self.assertIsNotNone(self.tbg_system.full_graph)
        self.assertGreater(len(self.tbg_system.full_graph.nodes), 0)
    
    def test_bilayer_structure(self):
        """Test that both layers are present with correct sublattice IDs."""
        sublattice_ids = {node.sublattice_id for node in self.tbg_system.full_graph.nodes}
    ...
    def test_interlayer_connections(self):
        ...
        for node in self.tbg_system.full_graph.nodes:
    ...
    def test_periodic_copy(self):
        ...
        periodic_graph = self.tbg_system.full_graph.create_periodic_copy(
            self.tbg_system.lattice_vectors, (1/3, 1/3)
        )
...
class test_numerical_stability(unittest.TestCase):
    ...
    def test_small_system_eigenvalues(self):
        ...
        tbg_instance = tbg(maxsize_n=1, maxsize_m=1, a=5, b=1, 
                 interlayer_dist_threshold=1.5,
                  intralayer_dist_threshold=1.5, unit_cell_radius_factor=1.0)
        
        periodic_graph = tbg_instance.full_graph.create_periodic_copy(
            tbg_instance.lattice_vectors, (0, 0)  # Gamma point
        )
        
        try:
            eigenvals,eigfun = periodic_graph.compute_bands_at_k(
                Momentum=constants.np.array([0.0, 0.0]), min_bands=1, max_bands=3,
            ...
            self.assertTrue(constants.np.allclose(constants.np.imag(eigenvals), 0, atol=1e-10))
